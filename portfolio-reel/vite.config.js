...
export default defineConfig({
  // @remotion/lambda is a Node.js server-side SDK — keep it out of the browser bundle.
  // The render.js file uses it only at runtime via a guarded import.
  build: {
    rollupOptions: {
  ...
  optimizeDeps: {
    exclude: ["@remotion/lambda", "@remotion/lambda/client"],
  },
  plugins: [
    react(),
    {
      ...
      configureServer(server) {
        ...
        server.middlewares.use("/api/render", (req, res) => {
          ...
          req.on("end", () => {
            ...
            try {
              const { props, compositionId = "PortfolioReel", scale = 1, codec = "h264" } = JSON.parse(body);
              ...
              const suffix = compositionId === "PortfolioReel" ? "9x16" : compositionId === "PortfolioReel_16x9" ? "16x9" : "1x1";
              const outPath = resolve(`out/reel-${suffix}.mp4`);
        ...
        // ── GET /api/download?suffix=9x16 — serve rendered file ──
        server.middlewares.use("/api/download", (req, res) => {
          const suffix = new URL(req.url, "http://localhost").searchParams.get("suffix") || "9x16";
          const outPath = resolve(`out/reel-${suffix}.mp4`);
